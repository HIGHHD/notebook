...
| ------------------------------ | ------------- | ----------------------------------------------------------- | ------------------------------ | ------------------------------ |
| linux centos7 8C/32G/1T        | 172.21.128.1 | docker环境搭建                                              | 是 | RAID5 |
| linux centos7 4C/8G/100G       | 172.21.128.2 | 文档中心前端、后台，目前192.168.1.203上的程序和191.168.1.171上的文档中心部署于该机器 | 否 |  |
| windows server 2008 2C/4G/100G       | 172.21.128.11 | 收益凭证登录接口部署                   | 否 |  |
| windows server 2016 4C/8G/100G | 172.21.128.21 | 唐宁的固收报表程序                                          | 否 |  |
| linux centos7 8C/16G/50G       | 172.21.130.1 | 数据接口系统，部署目前192.168.4.203上的程序，只提供kong访问 | 否 |  |
| linux centos7 8C/24G/300G      | 172.21.132.1 | 鲸准系统应用部署                                            | 否 |  |
| linux centos7 8C/16G/300G      | 172.21.132.2 | 鲸准系统postgresql、mongodb等数据库主用                     | 否 |  |
| linux centos7 2C/4G/300G       | 172.21.132.3 | 鲸准系统postgresql、mongodb等数据库冷备                     | 否 |  |
...
1. kong 和 数据接口系统，需要开通 172.21.128.1 访问 172.21.130.1:80 的访问控制
2. 数据接口系统至各数据库，需要开通172.21.130.1至各数据库的访问控制
3. 需要开通172.21.128.1到MariaDB的访问控制，有以下系统需要
   1. CMDB平台
   2. 首创企业微信消息应用系统
   3. 首创身份认证与授权管理应用
   4. 数据接口系统
   ...
   6. Redmine项目管理系统
   7. 报表中心（固定收益报表）
4. 首创基础消息应用系统需要访问Coremail邮件系统和玄武短信系统，需要开通172.21.128.1到 Coremail邮件系统和玄武短信系统的访问控制
5. 由于企业微信需要访问外网，172.21.128.1 需要开通访问外网的访问控制
6. CMDB和文档中心，以及下游的首创身份认证与授权管理应用，先通过VPN向各营业部提供服务，VPN需开通以下IP和端口
   1. 172.21.128.1 9968
   2. 172.21.128.1 9969
   3. 172.21.128.1 8088
   4. 172.21.128.2 80
7. 报表中心（固定收益报表）需访问衡泰利率资管系统温备库，需要开通 172.21.128.1 到 衡泰利率资管系统温备库的访问控制
8. 文档中心后台，提供了官网同步程序，需开通 172.21.128.2 到MariaDB集群的访问控制
9. 监控运维平台调用了首创企业微信消息应用系统，需要开通监控运维平台到 172.21.128.1 8012的访问控制
10. 收益凭证网站使用域名`service.sczq.com.cn`对外网发布，需开通公用域名服务到  172.21.128.1 8070 的访问控制
11. 首正泽富鲸准股权投资业务系统使用VPN对外发布，VPN需开通VPN需开通以下IP和端口
    1. 172.21.132.1 18080
    2. 172.21.132.1 9000
12. CMDB、首创身份认证与授权管理应用 调用AD域验证用户登录，需开通172.21.128.1 到 AD域的访问控制
13. CMDB访问OA，需要开通172.21.128.1到OA的访问控制
14. 隔离墙系统访问OA、聚源资讯，需要开通172.21.128.1到OA和聚源资讯的访问控制
...
server:
  authentication:
    url: http://172.21.128.1:9968/jwt/v1/auth/login
    check-url: http://172.21.128.1:9968/jwt/v1/auth/verify
    resource-url: http://172.21.128.1:9969/auth/v1/auth/loadResources
    authorize-url: http://172.21.128.1:9969/auth/v1/auth/authorize
```
