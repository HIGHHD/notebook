...
Mougodb登录数据库：
/data/webserver/mongodb/bin/mongo  192.168.1.120:27017/admin -u admin -p admin
...
在/data/backup/下执行：
wget https://vcmanager.jingdata.com/file/upgrade/nfdw/sql.tar.gz        //数据文件
wget https://vcmanager.jingdata.com/file/upgrade/nfdw/szzf_work.tar.gz   //服务文件
解压:
tar -xvf /data/backup/szzf_work.tar.gz
tar -xvf /data/backup/sql.tar.gz

微服务初始化： sh /data/shell/chushihua_work.sh，依照要求输入数据库及域名相关信息。
请输入 新库 用户(当前 'szzf'):szzf
请输入 新库 密码(当前 'szzf'):szzf
请输入 PG库地址(当前'postgres-server'):192.168.1.120
请输入 库名称(当前 'szzf') :szzf
请输入 库端口(当前 5432) :5432
请输入 新域名 (当前 'szzf.jingdata.com'):192.168.1.121
...
---
需要注意：
1、配置文件中存在 192.168 未配置项；
2、hosts文件里添加应用服务的地址解析；
3、filestore服务配置端口；
4、T的环境，metadata服务配置文件中替换open-api-admin.jingdata.com为open-api.jingdata.com；
6、nginx配置文件里的IP地址替换；
---
...
导入postgresql数据：/data/webserver/postgresql/bin/psql -h 192.168.1.120 -U szzf -p 5432  -d szzf  -f /data/backup/szzf.sql

导入mongodb数据：/data/webserver/mongodb/bin/mongorestore -h 192.168.1.120:27017  -u admin -p admin --dir /data/backup/
...
目前是
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJBUFAiLCJ1c2VyX2lkIjoiNjAxNzY3ZGIwMzc2YTI0MzY3NjA0NGU1OjYwMTc2ODdhMDM3NmEyNDM2NzYwNDg3MToxMzAwMDAwMDAwMCIsImlzcyI6IkppbmdkYXRhIiwiZXhwIjoxNjMyNjM0OTE4LCJpYXQiOjE2MzE3NzA5MTh9.xBNdwnUq4ae2IBNyaU_yok53DIGlDhSftnbTlc8ly2A

应用服务器终端执行(注意替换token)：
curl -X POST -H "Content-Type:application/json" -b "saas_login_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJBUFAiLCJ1c2VyX2lkIjoiNjAxNzY3ZGIwMzc2YTI0MzY3NjA0NGU1OjYwMTc2ODdhMDM3NmEyNDM2NzYwNDg3MToxMzAwMDAwMDAwMCIsImlzcyI6IkppbmdkYXRhIiwiZXhwIjoxNjMyNjM0OTE4LCJpYXQiOjE2MzE3NzA5MTh9.xBNdwnUq4ae2IBNyaU_yok53DIGlDhSftnbTlc8ly2A" -d '{"between":"","em":false,"entityIds": [],"tenants": ["601767db0376a243676044e5"]}' "http://jingdata-saas-investment:8000/api/search/synchroDataAllTenant"
